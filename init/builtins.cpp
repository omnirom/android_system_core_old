...
static int do_class_start(const std::vector<std::string>& args) {
    ...
    std::string prop_name = android::base::StringPrintf("class_start:%s", args[1].c_str());
...
static int do_class_stop(const std::vector<std::string>& args) {
    ...
    std::string prop_name = android::base::StringPrintf("class_stop:%s", args[1].c_str());
    if (prop_name.length() < PROP_NAME_MAX) {
        ActionManager::GetInstance().QueueEventTrigger(prop_name);
    }
    return 0;
...
static int do_class_reset(const std::vector<std::string>& args) {
    ...
    std::string prop_name = android::base::StringPrintf("class_reset:%s", args[1].c_str());
    if (prop_name.length() < PROP_NAME_MAX) {
        ActionManager::GetInstance().QueueEventTrigger(prop_name);
    }
    return 0;
